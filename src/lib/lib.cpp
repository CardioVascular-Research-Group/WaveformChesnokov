...
int  Signal::PopulateSignal(char *name)
{
	...
	samparray = (WFDB_Sample *)malloc(nsig * sizeof(WFDB_Sample));
	siarray = (WFDB_Siginfo *)malloc(nsig * sizeof(WFDB_Siginfo));
	nsig = isigopen(name, siarray, nsig);
	freq = getifreq();
...
//		pDataheader->hdr='DATA';
		...
		pDataheader->umv = siarray[sig].gain; //  ADC gain (ADC units per physical unit)
		pDataheader->bits = siarray[sig].adcres;//ADC resolution (bits)
		pDataheader->sr = freq;
		pDataheader->bline = siarray[sig].baseline; // baseline (ADC units) [optional]
		int msec = siarray[sig].nsamp * (1000/(int)freq);
		int ms;
		mSecToTime(msec, hh, mm, ss, ms);
		pDataheader->hh=hh; // hours
		pDataheader->mm=mm; // minutes
		pDataheader->ss=ss; // seconds
		pDataheader->lead = sig; // Lead number this data header represents.

		Len = pDataheader->size;
	    SR = pDataheader->sr;
	    Bits = pDataheader->bits;
	    Lead = pDataheader->lead;
	    UmV = pDataheader->umv;
	    hh = pDataheader->hh;
	    mm = pDataheader->mm;
	    ss = pDataheader->ss;
	    dat = true;
		hdrs.push_back(*pDataheader);

		data = new long double[Len];
//		data = new WFDB_Sample[Len];
...
////		getvec This function reads a sample from each input signal.
////		Return:
...
//		if (getvec(samparray) < 0)
//			break;

	sample = 0;
	while (getvec(samparray) >= 0){
//		if (sample <10){printf("%d ms\t",sample*(1000/(int)freq) ); }
		for (sig = 0; sig < nsig; sig++){
			vdata[sig][sample] = samparray[sig];
//			if (sample <10){
//				WFDB_Sample dum = vdata[sig][sample];
//				printf("%d|%d\t", samparray[sig], dum );
//			}
		}
//		if (sample <10){ printf("\n"); }
		sample++;
...
bool Signal::ReadRdt()
{
	...
	vector<long double> vec1, vec2, vec3;
	short channels, samplingRate, value;
	ifstream::pos_type fileSize;
	bool verbose = true;

	char* rdtfname = new char[wcslen(fname) + 1];
	wcstombs( rdtfname, fname, wcslen(fname) );
	char* rdtfile = new char[strlen(rdtfname)];
	getfname(rdtfname,rdtfile);
	changeext(rdtfile,".rdt");
	ifstream file (rdtfile, ios::in|ios::binary|ios::ate);
	...
	fread(&channels, sizeof(short), 1, in);
	printf("Channels: %u\n", channels);
	fread(&samplingRate, sizeof(short), 1, in);
	printf("Sampling Rate: %u\n", samplingRate);
	SR = samplingRate;
	int HEADERBYTES = 2*sizeof(short);
	int REALBUFFERSIZE = (int) fileSize - HEADERBYTES;

	if (REALBUFFERSIZE % (channels * sizeof(short)) != 0) {
		printf("rdt file is not aligned.\n");
		fclose(in);
		return false;
	}

	Len = REALBUFFERSIZE / (channels * sizeof(short));
	...
	for (int j = 0; j < Len; j++) {
		for (int i = 0; i < channels; i++) {
			fread(&value, sizeof(short), 1, in);
			if (i == 0) vec1.push_back((long double)value);
			if (i == 1) vec2.push_back((long double)value);
			if (i == 2) vec3.push_back((long double)value);
		}
	}
	...
	data = new long double[Len];
	for(int i=0; i<Len; i++) data[i] = vec1[i];
	DATAHDR hdr1;
	memset(&hdr1,0,sizeof(DATAHDR));
	hdr1.sr = SR;
	hdr1.size = Len;
	hdr1.umv = 200;
	hdr1.bits = 16;
	...
	data = new long double[Len];
	for(int i=0; i<Len; i++) data[i] = vec2[i];
	DATAHDR hdr2;
	memset(&hdr2,0,sizeof(DATAHDR));
	hdr2.sr = SR;
	hdr2.size = Len;
	hdr2.umv = 200;
	hdr2.bits = 16;
	...
	data = new long double[Len];
	for(int i=0; i<Len; i++) data[i] = vec3[i];
	DATAHDR hdr3;
	memset(&hdr3,0,sizeof(DATAHDR));
	hdr3.sr = SR;
	hdr3.size = Len;
	hdr3.umv = 200;
	UmV = hdr3.umv;
...
{
//	const char name2[512];
//	sprintf(name2, "%s", name);
	char *name2;
	wcstombs(name2,name,12);
	in = fopen(name2,"w");
	if(in)
	{
...
void Signal::getfname(char *path, char *name)
{
...
void Signal::changeext(char *path, char *ext)
{
 ...
 *
 * @param msec - input, time in milliseconds.
 * @param h - output, Hours
 * @param m - output, Minutes
 * @param s - output, Seconds
 * @param ms - output, remaining milliseconds
 */
...
bool FWT::InitFWT(char *fltname, long double *sig, int len)
{
//	char *name2;
//	wcstombs(name2,fltname,12);
//
	filter = fopen(fltname,"rt");
...
int ** Annotation::GetPTU(long double *mass, int len, double sr, char *fltdir, int **ann, int qrsnum)
{
	...
	for(int n=0; n<qrsnum-1; n++)
	{
		...
		//		a1 = ann[n*2][0];
		//		a2 = ann[n*2+2][0];
		//        rr = (long double)(a2-a1)/sr;
		// rr = 60.0 / ((r2-r1)/sr);
		//rr = (r2-r1)*(1000/sr);
		// 		 if(n<100){
		//			printf( "%i] rr5: %f, a1: %i, a2: %i, sr: %f\n",n, rr, a1, a2, sr);
		//		 }
...
bool Annotation::SaveQTseq(char *name, int **ann, int annsize, long double sr, int len)
{
...
bool Annotation::GetQTseq(int **ann, int nums, double sr, vector<long double> *QT, vector<int> *QTpos)
{
	...
	{
		if(ann[i][1]==15 || ann[i][1]==17) qNum++;
		else if(ann[i][1]==44 || ann[i][1]==45) tNum++;
	}
...
bool Annotation::SavePQseq(char *name, int **ann, int annsize, long double sr, int len)
{
...
bool Annotation::SavePPseq(char *name, int **ann, int annsize, long double sr, int len)
{
...
bool Annotation::GetRRseq(int **ann, int nums, double sr, vector<long double> *RR, vector<int> *RRpos)
{
	...
	long double r1,r2; // sample number of R peaks used to calculate HR. Calculation adjusts for sampling rate (sr).
...
//	wcscat(filter,L"/daub2.flt");
	strcpy(filter,fltdir);
	strcat(filter,"/daub2.flt");
