...
typedef struct _datahdr
{
   ...
   float sr; // sampling Rate/frequency (in samples per second per signal)
   unsigned char bits;//ADC resolution (bits)
   unsigned char lead;// Lead number this data header represents.
   unsigned short umv;//  ADC gain (ADC units per physical unit)
   unsigned short bline; // baseline (ADC units) [optional]
...
class Signal
{
  ...
  protected:
    ...
    int Len;// Number of samples per Lead

    int hh,mm,ss;               //start time of the record

      HANDLE fp, fpmap;
//         LPVOID lpMap;
      	  void* lpMap; // typedef void far            *LPVOID;
 ...
 unsigned short *lpu;
  ...
  long double *PopulateSignal(char *name);
  ...
  long double *ReadFile(char *name);
	  ...
	  void CloseFile();
 		...
 		/** data from a single lead/signal, as specified by the GetData( leadNumber ) method **/
 long double *GetData(){ return data; };
		/** Number of samples per Lead, same as Size. Was different per lead, now is assumed that all leads have the same number of samples. **/
         int GetLength(){ return Len; };
         /** ADC resolution (bits) **/
         int GetBits(){ return Bits; };
         /** ADC gain (ADC units per physical unit) **/
         int GetUmV(){ return UmV; };
         /** Lead number that GetData() currently represents. **/
         int GetLead(){ return Lead; };
         /** number of leads/signals in the vdata array **/
         int GetLeadsNum(){return (int)vdata.size(); };
         /**Sampling Rate/frequency (in samples per second per signal)**/
...
//      bool InitFWT(wchar_t *fltname, long double *sig, int len);
      bool InitFWT(char *fltname, long double *sig, int len);
      void CloseFWT();
...
//     bool SavePPseq(wchar_t *, int **, int, long double, int);
     bool SaveQTseq(char *, int **, int, long double, int);
     bool SavePQseq(char *, int **, int, long double, int);
     bool SavePPseq(char *, int **, int, long double, int);
};
